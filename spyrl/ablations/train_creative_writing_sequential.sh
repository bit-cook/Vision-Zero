#!/usr/bin/env bash
# Ablation: creative writing (WritingPrompts) with a *sequential* performing stage.
#
# Unlike spyrl/train_creative_writing.sh, players write one after another and each sees
# the stories already written; the spy writes last and receives no prompt at all, so it
# must infer the topic entirely from its peers.
set -xeo pipefail
source "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/env.sh"
spyrl_setup_experiment "${EXPERIMENT_NAME:-writingprompts_creative_writing_sequential}"

python3 -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    data.custom_cls.path="${SPYRL_DATASET_DIR}/writingprompts_spotdiff_dataset.py" \
    data.custom_cls.name=WritingPromptsSpotDiffTwoPlayerDataset \
