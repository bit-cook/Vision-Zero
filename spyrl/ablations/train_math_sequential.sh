#!/usr/bin/env bash
# Ablation: mathematical reasoning (Nemotron-CC-Math) with a *sequential* performing stage.
#
# Unlike spyrl/train_math_reasoning.sh, players speak in turn and each sees the problems already
# posed. The degradation is total rather than partial: the spy sees no document at all and
# must fabricate one from its peers' problems. It speaks last.
set -xeo pipefail
source "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/env.sh"
spyrl_setup_experiment "${EXPERIMENT_NAME:-nemotron_math_reasoning_sequential}"

python3 -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    data.custom_cls.path="${SPYRL_DATASET_DIR}/nemotron_cc_math_spotdiff_dataset.py" \
    data.custom_cls.name=NemotronCCMathSpotDiffTwoPlayerDataset \
