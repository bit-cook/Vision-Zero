#!/usr/bin/env bash
# SpyRL on creative writing (WritingPrompts), parallel performing stage.
#
# Civilians see the full writing prompt, the spy sees the same prompt with a random
# contiguous 20% span masked out. All players write independently, then vote on who is the
# spy.
set -xeo pipefail
source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/env.sh"
spyrl_setup_experiment "${EXPERIMENT_NAME:-writingprompts_creative_writing}"

python3 -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    data.custom_cls.path="${SPYRL_DATASET_DIR}/writingprompts_spotdiff_parallel_dataset.py" \
    data.custom_cls.name=WritingPromptsSpotDiffParallelDataset \
    ...
    actor_rollout_ref.rollout.agent.default_agent_loop=writingprompts_parallel \
    ...
    trainer.project_name='spyrl_writingprompts' \
