#!/usr/bin/env bash
# SpyRL on mathematical reasoning (Nemotron-CC-Math), parallel performing stage.
#
# Civilians see a math-heavy web document, the spy sees the same document with a random
# contiguous 40% span masked out. All players design and solve a problem grounded in what they
# were given, independently, then vote on who is the spy.
#
# Memory settings below are inherited from the sequential run
# (spyrl/ablations/train_math_sequential.sh). If you hit OOM, lower NUM_PLAYERS to 3 or drop
# actor_rollout_ref.rollout.n.
set -xeo pipefail
source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/env.sh"
spyrl_setup_experiment "${EXPERIMENT_NAME:-nemotron_math_reasoning}"

python3 -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    data.custom_cls.path="${SPYRL_DATASET_DIR}/nemotron_cc_math_spotdiff_parallel_dataset.py" \
    data.custom_cls.name=NemotronCCMathSpotDiffParallelDataset \
    ...
    +data.mask_fraction="${MASK_FRACTION:-0.4}" \
    actor_rollout_ref.rollout.agent.default_agent_loop=nemotron_cc_math_parallel \
