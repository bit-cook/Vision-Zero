...
"""SpyRL agent loop for Nemotron-CC-Math reasoning with a parallel performing stage.

One rollout plays one full game. In the performing stage every player designs and solves a math
problem concurrently from its own private view of the document; in the detection stage a detector
reads the full document plus all problems and votes for the spy.
...
from verl.utils.dataset.nemotron_cc_math_spotdiff_parallel_dataset import (
    NemotronCCMathSpotDiffParallelPromptBuilder,
)
...
@register("nemotron_cc_math_parallel")
class NemotronCCMathParallelClueDecisionAgentLoop(AgentLoopBase):
    """Parallel clue -> decision agent loop for Nemotron-CC-Math spot-the-difference."""
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        with simple_timer("generate_sequences", metrics):
            if rollout_n > 0 and cache_key in _CLUE_CACHE:
                ...
                math_data = game_data.get("nemotron_cc_math_data", {}) or {}
                full_document = str(math_data.get("document_text", ""))
                masked_document = str(math_data.get("masked_document_text", ""))
                mask_fraction = float(math_data.get("mask_fraction", 0.4))
                ...
                player_document_content = [full_document] * num_players
                player_document_content[spy_player - 1] = masked_document
                game_data["spy_player"] = spy_player
                game_data["player_document_content"] = player_document_content

                for round_num in range(1, num_rounds + 1):
                    civ_prompt = NemotronCCMathSpotDiffParallelPromptBuilder.build_clue_prompt_parallel(
                    ...
                    spy_prompt = NemotronCCMathSpotDiffParallelPromptBuilder.build_clue_prompt_parallel(
                        num_players=num_players,
                        round_num=round_num,
                        document_text=masked_document,
                        is_spy=True,
                        mask_fraction=mask_fraction,
                    )
                    ...
                    for response, prompt_ids, response_ids in civ_results:
                        clue_text = NemotronCCMathSpotDiffParallelPromptBuilder.extract_clue_answer(response)
                        civ_outputs.append(
                            {
                                "prompt": civ_prompt,
                                "prompt_ids": prompt_ids,
                    ...
                    spy_clue_text = NemotronCCMathSpotDiffParallelPromptBuilder.extract_clue_answer(spy_response)
                    ...
                    for idx, player_id in enumerate(civ_player_ids):
                        ...
                        clue_texts[(round_num, player_id)] = entry["clue_text"]
                        clue_sequence.append(
                            {"round": round_num, "player_id": player_id, "clue_text": entry["clue_text"]}
                        )
                    ...
                    clue_texts[(round_num, spy_player)] = spy_clue_text
                    clue_sequence.append({"round": round_num, "player_id": spy_player, "clue_text": spy_clue_text})

                all_clues = NemotronCCMathSpotDiffParallelPromptBuilder.build_all_clues_text_from_sequence(
                    clue_sequence, num_players, num_rounds
                )
                decision_prompt = NemotronCCMathSpotDiffParallelPromptBuilder.build_god_decision_prompt(
                    num_players=num_players,
                    document_text=full_document,
                    all_clues=all_clues,
                    mask_fraction=mask_fraction,
