...
"""SpyRL summarization environment on GovReport -- parallel performing stage.

Every player receives the same government report, except the spy whose copy has a random
contiguous span (``mask_fraction``, 0.2 by default) replaced by ``*``. Players write their
summaries independently: nobody sees anyone else's output before the detection stage, matching
the performing stage of Algorithm 1 in the paper.

Source: ``ccdv/govreport-summarization``, streamed; the first ``GOVREPORT_TAKE_N`` (default
20000) samples are materialized for random access. Paired with the ``govreport_parallel`` agent
loop.
"""
...
class GovReportSpotDiffParallelPromptBuilder:
    @staticmethod
    ...
    def build_clue_prompt_parallel(
        *,
        ...
        """
        Parallel clue stage: each player writes ONE paragraph summary independently.
        - Civilians see the full report.
        - Spy sees a report with a random contiguous masked span.
        - Players do NOT see other players' summaries in this stage.
        """
        ...
        game_context = f"""You are a player of the game "Who's the Odd One Out?". This is ROUND {round_num}. Your task is to write ONE paragraph summary of the report.
...
• Non-spy players see the full report. The spy sees a report where a random contiguous {percent}% span is masked with '*'.
• In this stage, players write summaries independently and cannot see other players' summaries.
{role_instruction}
...
2. If you are the spy:
    1) Use only your partial report view and do not invent missing details.
...
• Do not speculate about other players' summaries; focus only on your report.
...
Game Setup:
• There are {num_players} players in total. Only one player (the spy) saw a report where a random contiguous {percent}% span was masked with '*'. All other players saw the SAME full report.
• Each player wrote a summary independently without seeing other players' summaries.
• The spy tries to blend in by making their partial summary look as faithful as possible.
...
5. **Inconsistency with others**: Non-spy summaries tend to converge on the same report-grounded facts; a spy may drift or omit content.
...
class GovReportSpotDiffParallelDataset(Dataset):
    """
    Parallel spot-the-difference dataset using `ccdv/govreport-summarization`.

    Civilians see the sample's `report` in full.
    The spy sees the same report with a random contiguous span masked with '*'.
    """
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        report_text_full = GovReportSpotDiffParallelPromptBuilder.truncate_to_first_n_whitespace_tokens(
            report_text_full, self.report_max_tokens
        )

        rng = random.Random(self.seed + dataset_idx)
        spy_player = rng.randint(1, self.num_players)
        game_id = f"govreport_parallel_sample_{dataset_idx}"

        mask_info = GovReportSpotDiffParallelPromptBuilder.mask_random_contiguous_span(
            report_text_full, seed=self.seed + dataset_idx, fraction=self.mask_fraction
        )
        ...
        return {
            "prompt": messages,
            "raw_prompt": messages,
            "data_source": "govreport_spotdiff_parallel",
