...
class RayPPOTrainer:
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        if self.use_legacy_worker_impl == "disable":
            ...
            effective_rollout_n = batch.meta_info.get(
                "effective_rollout_n", self.config.actor_rollout_ref.rollout.n
            )
            ppo_mini_batch_size = ppo_mini_batch_size * int(effective_rollout_n)
    ...
    def _get_interactive_training_phase(self) -> str:
        phase = str(self.config.trainer.get("training_phase", "decision"))
        if phase == "interactive":
            cycle_length = int(self.config.trainer.get("interactive_cycle_length", 1))
            cycle_length = max(1, cycle_length)
            step_index = max(0, self.global_steps - 1)
            cycle_pos = step_index % (cycle_length * 2)
            if cycle_pos < cycle_length:
                return "decision"
            return "clue"
        if phase in {"clue", "decision", "both"}:
            return phase
        return "decision"
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                training_phase = self._get_interactive_training_phase()
                gen_batch.non_tensor_batch["training_phase"] = np.array(
                    [training_phase] * len(gen_batch), dtype=object
                )
                ...
                if training_phase == "clue":
                    repeat_times = max(
                        1, int(self.config.data.num_players) * int(self.config.data.num_rounds)
                    )
                gen_batch.meta_info["effective_rollout_n"] = repeat_times
                batch.meta_info["effective_rollout_n"] = repeat_times
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # repeat to align with repeated responses in rollout (phase-dependent)
